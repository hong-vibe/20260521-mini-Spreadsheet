...
### 🧠 어려웠던 점 및 해결 방법
* 스프레드시트의 2차원 배열 셀 값을 보존하는 'Undo(되돌리기) 스택'과 실시간 '로컬스토리지 자동 저장'의 주기 싱크를 완벽히 맞추는 부분이 까다로웠습니다. 셀이 변경되거나 삽입삭제되는 모든 라이프사이클 직전에 원본 스냅샷을 깊은 복사하여 히스토리 상자에 차곡차곡 보관하고, 되돌리기가 활성화되는 즉시 화면을 갱신하며 로컬스토리지도 실시간 오토세이브되도록 구조화하여 극복했습니다.

### 🔮 다음 번에 더 개선해 보고 싶은 점
* **자동 수식 파싱 엔진 구현**: 셀 내부에서 `=SUM(A1:A5)`이나 `=AVERAGE(B1:B9)` 같은 엑셀 수식을 타이핑했을 때 동적으로 값을 파싱하고 결과를 뱉어내도록 정밀 계산엔진을 추가해 보고 싶습니다.
* **대용량 가상 스크롤 기법 도입**: 지금은 미니 스프레드시트 컨셉이지만, 향후 1만 행 이상의 대단위 그리드에서도 메모리 부하와 렉이 걸리지 않도록 화면에 보이는 셀만 동적 렌더링하는 가상 스크롤(Virtual Scroll)을 구현해 보고 싶습니다.
